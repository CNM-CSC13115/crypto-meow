...
contract MeowFactory is MeowContract, MeowAdmin {
    ...
    // Manually manage the array length and store the new kitty
    ...
    function readyToBreed(uint256 _kittyId) public view returns (bool) {
        return kitties[_kittyId].cooldownEndTime <= block.timestamp;
    }

    function _setBreedCooldownEnd(Kitty storage _kitty) internal {
        _kitty.cooldownEndTime = uint64(
            block.timestamp.add(cooldowns[_kitty.cooldownIndex])
        );
    }
