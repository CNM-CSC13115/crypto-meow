...
const BtnSecondary = styled(Button)`
  background-color: #fd9bb3 !important;
  border-color: #fd9bb3 !important;
  width: 100%;
  font-weight: 700;
`;
  ...
  return (
    <Accordion>
      <PlainCard>
        <CardHeader>
          <Accordion.Toggle as={BtnSecondary} eventKey="0">
            <strong>
              <span>
                #{cat.kittyId} Gen {cat.generation}{" "}
                <span role="img" aria-label="timer clock">
                  ⏲
                </span>
                <span>
                  {cat.cooldown?.name} ({cat.cooldown?.durationName})
                </span>
              </span>
            </strong>
            <DnaViewer dna={dna} />
      ...
      <Accordion.Collapse eventKey="0">
        <Card.Body>
          {breedCountdown}
          <div className="d-flex flex-column text-lg">{cattributes}</div>
        </Card.Body>
      </Accordion.Collapse>
    </Accordion>
  );
