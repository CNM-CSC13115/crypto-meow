...
export default function App() {
  ...
  if (wallet.network) {
    unsupportedNetwork = onSupportedNetwork ? null : (
      <Alert variant="danger">
        Network {wallet.network.name} not supported. Please connect to{" "}
        {supportedNetworks.map((n) => n.name).join(", ")} to get started.
      </Alert>
    );
  }
