...
const { BN } = web3.utils;
const TestKittyContract = artifacts.require("TestMeowContract");
...
contract("MeowContract", (accounts) => {
  let contract;
  let kittyOwner;
  let kitty;
  let newOwner;
  ...
  function addApproval(cat, approved) {
    return contract.approve(approved, cat.kittyId, { from: cat.owner });
  }
  ...
  describe("init", () => {
    it("should be created with the un-kitty so valid kitties have an id > 0", async () => {
      ...
      expect(result.kittyId.toString(10)).to.equal(
        unKitty.kittyId.toString(10)
      );
      expect(result.birthTime.toString(10)).to.equal(
        unKitty.birthTime.toString(10)
      );
      expect(result.cooldownEndTime.toString(10)).to.equal(
        unKitty.cooldownEndTime.toString(10)
      );
      ...
      expect(result.generation.toString(10), "generation").to.equal(
        unKitty.generation.toString(10)
      );
      expect(result.cooldownIndex.toString(10), "generation").to.equal(
        unKitty.cooldownIndex.toString(10)
      );
  ...
  describe("transfer", () => {
    ...
    it("should emit a Transfer event", async () => {
      const result = await contract.transfer(newOwner, kitty.kittyId, {
        from: kittyOwner,
      });
    ...
    it("should NOT revert if the sender is NOT the owner but IS approved", async () => {
      ...
      await truffleAssert.passes(
        contract.transfer(approvedAddress, kitty.kittyId, {
    ...
    it("should NOT revert if the send is NOT the owner but IS an approved operator", async () => {
      ...
      await contract.setApprovalForAll(operator, true, { from: kittyOwner });

      await truffleAssert.passes(
        contract.transfer(operator, kitty.kittyId, { from: operator })
      );
    });
  ...
  describe("approve", () => {
    ...
    it("should set an approval for the given address", async () => {
      await contract.approve(approvedAddr, kitty.kittyId, {
        from: kitty.owner,
      });
    ...
    it("should emit an Approval event", async () => {
      const result = await contract.approve(approvedAddr, kitty.kittyId, {
        from: kitty.owner,
      });
  ...
  describe("Operator approval for all", () => {
    it("should set and revoke operator approval for an address", async () => {
      ...
      const result = await contract.isApprovedForAll(kittyOwner, operator);

      expect(result).to.equal(true);

      // revoke operator approval
      await contract.setApprovalForAll(operator, false, { from: kittyOwner });

      const result2 = await contract.isApprovedForAll(kittyOwner, operator);
    ...
    it("should support setting multiple operator approvals per address", async () => {
      ...
      await contract.setApprovalForAll(operator1, true, { from: kittyOwner });

      const result = await contract.isApprovedForAll(kittyOwner, operator1);

      expect(result).to.equal(true);

      // approve second operator
      const operator2 = accounts[5];
      await contract.setApprovalForAll(operator2, true, { from: kittyOwner });

      const result2 = await contract.isApprovedForAll(kittyOwner, operator1);
    ...
    it("should emit an ApprovalForAll event", async () => {
      ...
      const operator1 = accounts[4];
      const result = await contract.setApprovalForAll(operator1, true, {
        from: kittyOwner,
      });
  ...
  describe("transferFrom", () => {
    ...
    it("when the sender is the owner it should transfer ownership", async () => {
      await contract.transferFrom(kitty.owner, newOwner, kitty.kittyId, {
        from: kitty.owner,
      });
    ...
    it("when the sender is approved it should transfer ownership", async () => {
      ...
      await truffleAssert.passes(
        contract.transferFrom(kitty.owner, approved, kitty.kittyId, {
          from: approved,
        })
      );
    });
    ...
    it("when the sender is an approved operator it should transfer ownership", async () => {
      ...
      await truffleAssert.passes(
        contract.transferFrom(kitty.owner, operator1, kitty.kittyId, {
          from: operator1,
        })
      );
    });
    ...
    it("should REVERT when the sender is not the owner, approved, or an approved operator", async () => {
      ...
      await truffleAssert.reverts(
        contract.transferFrom(kitty.owner, unapproved, kitty.kittyId, {
          from: unapproved,
        })
      );
    });
    ...
    it("should REVERT if from address is not the owner", async () => {
      await truffleAssert.reverts(
        contract.transferFrom(newOwner, newOwner, kitty.kittyId, {
          from: kittyOwner,
        })
      );
    });
    ...
    it("should REVERT if to address is the zero address", async () => {
      await truffleAssert.reverts(
        contract.transferFrom(kitty.owner, zeroAddress, kitty.kittyId, {
          from: kittyOwner,
        })
      );
    });
    ...
    it("should REVERT if tokenId is not valid", async () => {
      const invalidTokenId = 1234;
      await truffleAssert.reverts(
        contract.transferFrom(kitty.owner, newOwner, invalidTokenId, {
          from: kittyOwner,
        })
      );
    });
  ...
  describe("safeTransferFrom", () => {
    ...
    it("should transfer ownership when the sender is the owner", async () => {
      await contract.safeTransferFrom(kitty.owner, newOwner, kitty.kittyId, {
        from: kitty.owner,
      });
    ...
    it("should transfer when the sender is NOT the owner but IS approved", async () => {
      ...
      await contract.safeTransferFrom(kitty.owner, approved, kitty.kittyId, {
        from: approved,
      });
    ...
    it("should REVERT when the sender is NOT the owner and NOT approved", async () => {
      ...
      await truffleAssert.reverts(
        contract.safeTransferFrom(kitty.owner, unApproved, kitty.kittyId, {
          from: unApproved,
        })
      );
    });
    ...
    it("should REVERT if the from address is not the owner", async () => {
      await truffleAssert.reverts(
        contract.safeTransferFrom(newOwner, newOwner, kitty.kittyId, {
          from: kitty.owner,
        })
      );
    });
    ...
    it("should REVERT if the to address is the zero address", async () => {
      await truffleAssert.reverts(
        contract.safeTransferFrom(kitty.owner, zeroAddress, kitty.kittyId, {
          from: kitty.owner,
        })
      );
    });
