...
const { BN } = web3.utils;
const TestKittyFactory = artifacts.require("TestMeowFactory");
const KittyFactory = artifacts.require("MeowFactory");

contract("MeowFactory", async (accounts) => {
  let kittyFactory;
  let contractOwner;
  let kittyOwner;
  let testAccount;
  ...
  async function addGen0Kitty(dna, _from = contractOwner) {
    return kittyFactory.createKittyGen0(dna, { from: _from });
  }
  ...
  describe("Create Gen 0 Kitty", () => {
    ...
    it("should store the new kitty", async () => {
      ...
      expect(result.generation.toString(10)).to.equal(
        expKitty.generation.toString(10)
      );
      expect(result.cooldownIndex.toString(10)).to.equal(
        expKitty.cooldownIndex.toString(10)
      );
  ...
  describe("breed", () => {
    ...
    async function breedParents() {
      return kittyFactory.breed(dad.kittyId, mum.kittyId, { from: kittyOwner });
    }
    ...
    it("should create a new kitty assigned to the sender", async () => {
      ...
      await kittyFactory.breed(dad.kittyId, mum.kittyId, { from: kittyOwner });

      const actualKitty = await kittyFactory.getKitty(expKitty.kittyId);

      expect(actualKitty.kittyId.toString(10)).to.equal(
        expKitty.kittyId.toString(10)
      );
      expect(actualKitty.mumId.toString(10)).to.equal(
        expKitty.mumId.toString(10)
      );
      expect(actualKitty.dadId.toString(10)).to.equal(
        expKitty.dadId.toString(10)
      );
      expect(actualKitty.generation.toString(10)).to.equal(
        expKitty.generation.toString(10)
      );
      expect(actualKitty.cooldownIndex.toString(10)).to.equal(
        expKitty.cooldownIndex.toString(10)
      );
    ...
    it("should emit a Birth event", async () => {
      ...
      const result = await kittyFactory.breed(dad.kittyId, mum.kittyId, {
        from: kittyOwner,
      });
    ...
    it("should REVERT if the sender does not own the dad kitty", async () => {
      ...
      await truffleAssert.reverts(
        kittyFactory.breed(dad.kittyId, mum.kittyId, { from: kittyOwner })
      );
    });
    ...
    describe("when siring", () => {
      beforeEach(async () => {
        ...
        dad.owner = accounts[3];
        await createParents();
        await kittyFactory.sireApprove(dad.kittyId, mum.kittyId, true, {
          from: dad.owner,
        });
      });
      ...
      it("should NOT revert if the sender does not own the dad kitty but is approved", async () => {
        ...
        await truffleAssert.eventEmitted(result, "Birth");
      });

      it("should reset the sire approval to false", async () => {
        await kittyFactory.breed(dad.kittyId, mum.kittyId, { from: mum.owner });
      ...
      describe("generation number", () => {
        [
          ...
          it(`should always be 1 higher than max of parents. Case: ${testCase.name} mum gen: ${testCase.mumGen} dad gen: ${testCase.dadGen}`, async () => {
            ...
            await kittyFactory.breed(dad.kittyId, mum.kittyId, {
              from: kittyOwner,
            });

            const actualKitty = await kittyFactory.getKitty(expKitty.kittyId);
            expect(actualKitty.generation.toString(10)).to.equal(
              testCase.kittyGen.toString()
            );
          });
        });
      });
    ...
    describe("parents", () => {
      describe("cooldown end time", () => {
        ...
        async function getExpCooldownEndTime(cooldownIndex) {
          ...
          const endMoment = moment.unix(now).add(cooldown);
          return {
            start: now,
        ...
        async function expectCoolDownTime(kittyId, cooldownIndex) {
          ...
          expect(parent.cooldownEndTime.toString(10)).to.equal(
            expTime.end.toString()
          );
        }
      ...
      describe("cooldown index", () => {
        async function expectCooldownIndex(expIndex) {
          [mum.kittyId, dad.kittyId].forEach(async (kittyId) => {
            ...
            expect(kitty.cooldownIndex.toString(10)).to.equal(
              expIndex.toString()
            );
          });
        }
      ...
      [{ name: "mum" }, { name: "dad" }].forEach((testCase) => {
        it(`should REVERT if the ${testCase.name} is on cooldown`, async () => {
          const kittyId = testCase.name === "mum" ? mum.kittyId : dad.kittyId;
  ...
  describe("sire approval", () => {
    ...
    it("should set approval to false", async () => {
      ...
      await kittyFactory.sireApprove(dad.kittyId, mum.kittyId, false, {
        from: dad.owner,
      });
    ...
    it("should REVERT if the sender does not own _dadId and is NOT approved", async () => {
      ...
      await truffleAssert.reverts(
        kittyFactory.sireApprove(dad.kittyId, mum.kittyId, true, {
          from: mum.owner,
        }),
        truffleAssert.ErrorType.REVERT,
        "approved"
    ...
    it("should NOT revert if the sender does not own _dadId but IS approved", async () => {
      await createGenXParents();
      await kittyFactory.setApprovalForAll(mum.owner, true, {
        from: dad.owner,
      });
