...
const KittyAdmin = artifacts.require("MeowAdmin");
const KittyAdminTest = artifacts.require("MeowAdminTest");

contract("MeowAdmin", (accounts) => {
  let contractOwner;
  ...
  describe("init", () => {
    ...
    it.only("should add the contract owner at index 1", async () => {
      ...
      const mappingResult =
        await contractInstance.test_getKittyCreatorFromMapping(contractOwner);
  ...
  describe("addKittyCreator", () => {
    it("should add a new address", async () => {
      ...
      const resultAddress =
        await contractInstance.test_getKittyCreatorFromArray(startingIndex);
      const resultId = await contractInstance.test_getKittyCreatorFromMapping(
        testCreator
      );

      expect(resultId.toString(10)).to.equal(
    ...
    it("should REVERT if the sender is not the Owner", async () => {
      await truffleAssert.reverts(
        contractInstance.addKittyCreator(testCreator, { from: accounts[2] }),
        "only owner"
      );
    });

    it("should emit a KittyCreatorAdded event", async () => {
      const result = await contractInstance.addKittyCreator(testCreator, {
        from: contractOwner,
      });
    ...
    it("should REVERT if address is the contract address", async () => {
      await truffleAssert.reverts(
        contractInstance.addKittyCreator(contractInstance.address, {
          from: contractOwner,
        }),
        "contract address"
      );
    ...
    it("should REVERT if the address is the zero address", async () => {
      await truffleAssert.reverts(
        contractInstance.addKittyCreator(zeroAddress, { from: contractOwner }),
        "zero address"
      );
    });
  });
  ...
  describe("removeKittyCreator", () => {
    ...
    it("should remove the address from the mapping", async () => {
      ...
      const result = await contractInstance.test_getKittyCreatorFromMapping(
        testCreator
      );
    ...
    it("should remove the address from the array", async () => {
      ...
      const result = await contractInstance.test_getKittyCreatorFromArray(
        startingIndex
      );
      expect(result).to.equal(zeroAddress);
    });
