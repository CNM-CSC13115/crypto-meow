...
const TestKittyMarketPlace = artifacts.require("TestMeowMarketPlace");
const TestKittyFactory = artifacts.require("TestMeowFactory");

contract("MeowMarketPlace", (accounts) => {
  let kittyFactory;
  ...
  async function initContracts() {
    ...
    await kittyMarketPlace.setKittyContract(kittyFactory.address, {
      from: contractOwner,
    });
  }
  ...
  function setOffer(price, kitty, fromAccount) {
    ...
    return kittyMarketPlace.setOffer(price, kitty.kittyId, { from: account });
  }

  function setSireOffer(price, kitty, fromAccount) {
    const account = fromAccount || kitty.owner;
    return kittyMarketPlace.setSireOffer(price, kitty.kittyId, {
      from: account,
    });
  }

  function buyKitty(kitty, buyer, _value) {
    return kittyMarketPlace.buyKitty(kitty.kittyId, {
      from: buyer,
      value: _value,
    });
  }
  ...
  function buySireRites(sire, buyer, _value, matron) {
    return kittyMarketPlace.buySireRites(sire.kittyId, matron.kittyId, {
      from: buyer,
      value: _value,
    });
  }
    ...
    describe("set kitty contract", () => {
      ...
      it("should REVERT if NOT called by the contract owner", async () => {
        await truffleAssert.reverts(
          kittyMarketPlace.setKittyContract(kittyFactory.address, {
            from: accounts[1],
          })
        );
      });
  ...
  describe("Market", () => {
    ...
    describe("Set offer", () => {
      function testSetOffer(offerFn, transType) {
        describe("common", () => {
          ...
          it(`should emit a MarketTransaction event with type "${transType}"`, async () => {
            ...
            expectMarketEvent(res, transType, kitty.owner, kitty.kittyId);
          });
      ...
      describe("Sire", () => {
        ...
        it("should REVERT if the sire is not ready to breed", async () => {
          ...
          await kittyFactory.breed(kitty.kittyId, mom.kittyId, {
            from: kitty.owner,
          });
      ...
      describe("Get Offer", () => {
        ...
        it("should REVERT if there is no active offer for that token", async () => {
          await truffleAssert.reverts(kittyMarketPlace.getOffer(kitty.kittyId));
        });
      });
    });
    ...
    describe("Remove Offer", () => {
      ...
      it("should set the offer.active to false", async () => {
        await kittyMarketPlace.removeOffer(kitty.kittyId, {
          from: kitty.owner,
        });
      ...
      it('should emit a MarketTransaction event of type "Remove"', async () => {
        const res = await kittyMarketPlace.removeOffer(kitty.kittyId, {
          from: kitty.owner,
        });

        expectMarketEvent(res, transTypes.remove, kitty.owner, kitty.kittyId);
      });

      it("should REVERT if sender is not the seller", async () => {
        const scammer = accounts[3];
        await truffleAssert.reverts(
          kittyMarketPlace.removeOffer(kitty.kittyId, { from: scammer })
        );
      });
    });
    ...
    describe("Execute offer", () => {
      ...
      function testExecuteOffer(offerFn, exeOfferFn, txType, matron) {
        describe("common", () => {
          ...
          it("should send funds to the seller", async () => {
            ...
            const sellerBalBefore = new BN(
              await web3.eth.getBalance(kitty.owner)
            );
            ...
            const sellerBalAfter = new BN(
              await web3.eth.getBalance(kitty.owner)
            );
            const expSellerBal = sellerBalBefore.add(expPrice);
            expect(sellerBalAfter.toString(10)).to.equal(
              expSellerBal.toString(10)
            );
          });
          ...
          it(`should emit a MarketTransaction event of type "${txType}"`, async () => {
            ...
            expectMarketEvent(result, txType, buyer, kitty.kittyId);
          });
      ...
      describe("Purchase sire rites", () => {
        ...
        it("should breed the sire and matron", async () => {
          ...
          await buySireRites(kitty, buyer, expPrice, mum);

          const kitten = await kittyFactory.getKitty(expKitten.kittyId);

          expect(kitten.mumId.toString(10)).to.equal(
            expKitten.mumId.toString(10)
          );
          expect(kitten.dadId.toString(10)).to.equal(
            expKitten.dadId.toString(10)
          );
        ...
        it("should REVERT if the matron is not ready to breed", async () => {
          ...
          await kittyFactory.breed(otherDad.kittyId, mum.kittyId, {
            from: buyer,
          });
    ...
    describe("Get all active offers", () => {
      ...
      async function expectOffers(expOffers, actualIds) {
        expect(actualIds.length).to.equal(
          expOffers.length,
